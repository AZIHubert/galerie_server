...
export default async (req: Request, res: Response) => {
  const {
    id: googleId, name, photoUrl, email,
  } = req.body;
  if (!googleId) {
    return res.status(400).send({
      errors: 'google id not found',
    });
  }
  if (!name) {
    return res.status(400).send({
  ...
  const defaultProfilePicture = photoUrl || null;
  if (!user) {
  ...
  return res.status(200).send({
    token: jwt.token, expiresIn: jwt.expires, session: req.sessionID,
  });
};
