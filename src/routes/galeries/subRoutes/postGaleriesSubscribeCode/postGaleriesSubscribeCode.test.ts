  ...
  describe('subscribe', () => {
    describe(':code', () => {
      describe('should return status 200', () => {
        ...
        it('and decrement numOfInvit', async () => {
          const numOfInvit = 2;
          const { body: { id: galerieId } } = await agent
            .post('/galeries')
            .set('authorization', token)
          ...
          if (invitation) {
            expect(invitation.numOfInvit).toEqual(numOfInvit - 1);
          }
        });
        it('and delete invitation if numOfInvit < 1', async () => {
          const numOfInvit = 1;
          const { body: { id: galerieId } } = await agent
            .post('/galeries')
            .set('authorization', token)
      ...
      describe('should return error 400', () => {
        it('if you are already subscribing to this galerie', async () => {
          const { body: { id: galerieId } } = await agent
            .post('/galeries')
            .set('authorization', token)
            .send({ name: 'galerie name' });
        ...
        it('and destroy invitation if numOfInvit is less than 1', async () => {
          const code = 'code';
          const { body: { id: galerieId } } = await agent
            .post('/galeries')
            .set('authorization', token)
      ...
      describe('should return error 404', () => {
        it('if invitation not found', async () => {
          const { body, status } = await agent
            .post('/galeries/subscribe/0')
            .set('authorization', token);
